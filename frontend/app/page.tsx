...
export default function SmartScheduler() {
    ...
    const check = () => {
      ...
      tasks.forEach(t=>{
        ...
        if(h === nowHour && m === nowMin){
          setNotif(`⏰ Reminder: ${t.subject} - Abhi ka time hai!`); setTimeout(()=>setNotif(null), 6000);
          if("Notification" in window && Notification.permission === "granted"){ new Notification(`Study Time: ${t.subject}`, { body: `${t.time} - ${t.duration}` }); }
  ...
  useEffect(() => {
    ...
    if (isRunning && pomodoroTime > 0) { interval = setInterval(() => setPomodoroTime(t => t - 1), 1000); }
      ...
      setShowCelebration(true); setCelebrationText(MOTIVATIONAL_QUOTES[Math.floor(Math.random()*MONTHS.length)]);
  ...
  const addTask = () => {
    if(!sub) return; const finalMin = String(parseInt(timeMin) || 0).padStart(2,'0'); const finalTime = `${timeHour}:${finalMin} ${ampm}`;
    ...
    <div className="bg-[#050711] text-white min-h-screen" suppressHydrationWarning>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 max-w-[1600px] mx-auto">
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-[#121424] p-5 rounded-2xl border border-white/5 transition-all duration-300 hover:-translate-y-2 hover:border-orange-500 hover:shadow-[0_20px_40px_-10px_rgba(249,115,22,0.6)]">
            ...
            <div className="bg-black/50 rounded-xl p-4 text-center border border-white/10 mb-4">
              ...
              <div className="flex justify-center gap-2 mt-4"><button onClick={()=>adjustTime(-1)} className="px-4 py-1.5 bg-white/10 rounded-full text-sm font-bold hover:bg-orange-500 transition-all">-1m</button><button onClick={()=>setIsRunning(!isRunning)} className="px-6 py-1.5 bg-gradient-to-r from-orange-500 to-amber-500 rounded-full text-sm font-bold hover:scale-110 transition-all">{isRunning? 'Pause':'Start'}</button><button onClick={()=>adjustTime(1)} className="px-4 py-1.5 bg-white/10 rounded-full text-sm font-bold hover:bg-orange-500 transition-all">+1m</button></div>
            ...
            <div className="flex gap-2"><input type="number" min="1" max="120" value={customMins} onChange={e=>setCustomMins(e.target.value)} className="w-[60%] bg-black/50 p-2.5 rounded-lg border border-white/10 text-center font-bold text-lg outline-none" /><button onClick={applyCustomTime} className="w-[40%] bg-white text-black font-bold rounded-lg hover:bg-orange-500 hover:text-white transition-all">Set Mins</button></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 bg-[#121424] p-5 rounded-2xl border border-white/5 min-h-[500px] transition-all duration-300 hover:border-emerald-500/50 hover:shadow-[0_20px_50px_-15px_rgba(16,185,129,0.4)]">
          <h2 className="font-bold text-xl mb-1">Study Schedule - {selectedDate.toLocaleDateString('en-US', {weekday:'short', month:'short', day:'numeric'})}</h2><p className="text-sm text-gray-400 mb-5">{filteredTasks.length} tasks</p>
        ...
        <div className="lg:col-span-3 bg-[#121424] p-5 rounded-2xl border border-white/5 h-fit transition-all duration-300 hover:-translate-y-2 hover:border-pink-500/50 hover:shadow-[0_20px_40px_-10px_rgba(236,72,153,0.5)] relative">
          <div className="flex justify-between items-center mb-4 gap-1">
            ...
            <div className="flex gap-1 relative">
              <div className="relative">
                <button onClick={()=>{setShowMonthList(!showMonthList); setShowYearList(false);}} className="bg-gradient-to-br from-[#1E213A] to-[#121424] border border-[#6C5CE7]/50 font-black text-[13px] px-3 py-2 rounded-xl shadow-[0_4px_15px_rgba(108,92,231,0.3)] hover:scale-110 hover:border-pink-500 hover:shadow-[0_0_20px_#EC4899] hover:from-pink-600/20 hover:to-purple-600/20 transition-all duration-300 transform-gpu">
                  {MONTHS[viewDate.getMonth()].slice(0,3)}
                </button>
                {showMonthList && (
                  <div className="absolute top-[110%] left-0 bg-[#0A0C1A] border border-[#6C5CE7]/50 rounded-2xl p-2 w-[150px] z-50 shadow-[0_20px_40px_rgba(0,0,0,0.8)] backdrop-blur-xl grid grid-cols-1 gap-1 max-h-[220px] overflow-y-auto">
                    {MONTHS.map((m,i)=>{
                      const isActive = i===viewDate.getMonth();
                      return <button key={m} onClick={()=>{const d=new Date(viewDate); d.setMonth(i); setViewDate(d); setShowMonthList(false);}} className={`text-left px-3 py-2 rounded-xl text-[12px] font-bold transition-all duration-200 transform-gpu hover:scale-[1.08] hover:translate-x-1 ${isActive? 'bg-gradient-to-r from-[#6C5CE7] to-[#EC4899] text-white shadow-[0_0_15px_#6C5CE7] scale-[1.05]' : 'bg-white/5 hover:bg-[#6C5CE7] hover:text-white hover:shadow-[0_0_15px_rgba(108,92,231,0.6)] text-gray-300'}`}>{m}</button>
                    })}
                  </div>
                )}
              </div>
              ...
              <div className="relative">
                <button onClick={()=>{setShowYearList(!showYearList); setShowMonthList(false);}} className="bg-gradient-to-br from-[#1E213A] to-[#121424] border border-pink-500/40 font-black text-[13px] px-3 py-2 rounded-xl shadow-[0_4px_15px_rgba(236,72,153,0.3)] hover:scale-110 hover:border-[#6C5CE7] hover:shadow-[0_0_20px_#6C5CE7] hover:from-purple-600/20 hover:to-pink-600/20 transition-all duration-300 transform-gpu ml-1">
          ...
          <div className="grid grid-cols-7 gap-1.5 text-[13px] text-center relative z-10">
            ...
            {Array.from({length: new Date(viewDate.getFullYear(), viewDate.getMonth()+1, 0).getDate()},(_,i)=>{
              ...
              const isSel = dObj.toDateString() === selectedDateStr; const hasTasks = tasks.some(t=>t.date === dObj.toDateString());
